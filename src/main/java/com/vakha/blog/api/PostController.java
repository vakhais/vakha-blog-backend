...
import com.vakha.blog.security.CurrentUser;
import com.vakha.blog.security.UserPrincipal;
import com.vakha.blog.service.PostService;
...
public class PostController {
    ...
    public ResponseEntity<?> getPostsByCategory(@PathVariable Long id, Pageable pageable) {
        Page<Post> posts = postService.findForCategoryIdOrderByCreatedDateDescPageable(id, pageable);
        Page<PostDto> postDto = posts.map(post -> new PostDto(post));
        return new ResponseEntity<>(postDto.getContent(), HttpStatus.OK);
    }

//    @GetMapping(value = "/posts/category/{name}")
...
//    }
    ...
    public ResponseEntity<PostDto> registerPost(@CurrentUser UserPrincipal currentUser,
                                                @RequestBody PostDto postDto) {
        log.debug("REST request to save Post : {}", postDto);
        if (postDto.getId() != null) {
            throw new ApiException("A new post cannot already have an ID", HttpStatus.CONFLICT);
        } else {
            PostDto returnPost = postService.registerPost(postDto, currentUser);
